...
def decoding(string, decodes):
    ...
    while i < len(string):
        ...
        for code in decodes:
            ...
            if string[i:].find(decodes[code]) == 0:
                res += code
                i += len(decodes[code])
...
my_string = input('Введите строку из трёх слов для сжатия: ')
...
print('Сжатая строка: ', coding(my_string, codes))
